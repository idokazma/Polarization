...
for ll=1:params.num_of_scatterers
    mid = length(E_SOL_st_pol_final(ll,:));
%     p1(ll) = plot(OMEGA_final,abs(E_SOL_st_MoM_final(ll,:)) - abs(E_SOL_st_MoM_final(ll,round(mid/2))) +abs(E_SOL_st_pol_final(ll,round(mid/2) )),'x-'); hold on;
%     p2(ll) = plot(OMEGA_final,abs(E_SOL_st_filaments_mul_final(ll,:))-abs(E_SOL_st_filaments_mul_final(ll,round(mid/2)))+abs(E_SOL_st_pol_final(ll,round(mid/2))),'s-');
    p3(ll) = plot(OMEGA_final,abs((E_SOL_st_pol_final(ll,:) )),'d-');hold on;
...
%     p2(ll) = plot(OMEGA_final,real(E_SOL_st_filaments_mul_final(ll,:) ),'s-');
    p3(ll) = plot(OMEGA_final,real((E_SOL_st_pol_final(ll,:) )),'d-'); hold on;
...
%     p2(ll) = plot(OMEGA_final,imag(E_SOL_st_filaments_mul_final(ll,:) ),'s-');
    p3(ll) = plot(OMEGA_final,imag((E_SOL_st_pol_final(ll,:) )),'d-'); hold on
...
figure; scatter(params_final.sca_x*1e6, params_final.sca_y*1e6,500,abs(E_SOL_st_pol_final(:,1)),'.');
colormap('jet');
