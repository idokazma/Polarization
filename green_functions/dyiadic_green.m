...
Gx_ = Gx_ ./(4*1i).*rot*fact ;

Gy_ = k0.*n.*(xtest-xsource).*besselh(1,1,k0*n*rr)./rr + 1i*k0*k0*OMEGA/omega*(ysource-ytest).*besselh(0,1,k0*n*rr);
Gy_ = Gy_ .*(-1/(4*1i)).*rot*fact ;

a=1;

Gx = 1i/4*k0*n./rr.*(ytest-ysource).*besselh(1,1,k0*n*rr).*rot + 1i*OMEGA/omega*k0^2*(xtest-xsource).*Gst.*rot;
Gy = 1i/4*k0*n./rr.*(-xtest+xsource).*besselh(1,1,k0*n*rr).*rot + 1i*OMEGA/omega*k0^2*(ytest-ysource).*Gst.*rot;
Gx = Gx/(1i*omega*mu)*Iz;
Gy = Gy/(1i*omega*mu)*Iz;
a=1;
